...
export default () => {
  ...
  return (
    <nav
      className = {`bg-white pb-5 md:text-em ${state ? "shadow-lg rounded-xl border mx-2 mt-2 md:shadow-none md:border-none md:mx-2 md:mt-0": "" }`}
    >
          ...
          >
            <ul className="justify-center items-center space-y-6 md:flex md:space-x-6 md:space-y-0">
              {navigation.map((item, idx) => {
                return (
                  <li key={idx} className="text-gray-700 hover:textgray-900">
            ...
            <div className='flex-1 gap-x-6 items-center justify-end mt-6 space-y-6 md:flex md:space-y-0 md:mt-0'>
              {currentUser ? (
                ...
                <button onClick={() => connectWallet()}
                        className="flex items-center justify-center gap-x-1 py-2 px-4 text-white font-medium bg-gray-800 hover:bg-gray-700 active:bg-gray-900 rounded-full md:incline-flex"
                >
                  Connect Wallet
                  <Nav3/>
                </button>
